...
# Have to insert data from 2008 because our scraper missed it, somehow
nfl_df1 = nfl_df.iloc[:32, :]

nfl_df2 = nfl_df.iloc[32:, :]

nfl_df3 = pd.DataFrame([['New York Giants', 'Feb 03, 2008', "New York Giants,'BIG DIP", 'NASDAQ', 'Feb 05, 2008', -.03147, 'BIG DIP']], columns = nfl_df.columns)

nfl_df3 = pd.DataFrame([['New York Giants', 'Feb 03, 2008', "New York Giants,BIG DIP", 'NASDAQ', 'Feb 05, 2008', -.03147, 'BIG DIP']], columns = nfl_df.columns)

nfl_df3 = pd.DataFrame([['New York Giants', 'Feb 03, 2008', "New York Giants,BIG DIP", 'NASDAQ', 'Feb 05, 2008', -.03147, 'BIG DIP']], columns = nfl_df.columns)

nfl_df = pd.concat([nfl_df1, nfl_df3, nfl_df2])
...
####
# Visual depictions of our findings from the network analyses
####

postSB = nfl_df['Stock Percent Change']

postSB = [100 * x for x in postSB]

years = list(range(1976, 2018))


helper = list(stocks['STURM & RUGER (GUNS) Percent Change'])[::-1][2000:]

helper = [100*x for x in helper]

trace60 = go.Scatter(name = 'Percent Change After Super Bowl', y = helper,
                     mode = 'lines')

layout = dict(title = 'Daily Percentage Changes in Sturm & Ruger',
              xaxis = dict(title = 'Days from Start'),
...
postSB2 = nfl_df['Stock Change Category']


postSB2 = ['BIG DIP', 'DIP', 'LITTLE MOVEMENT', 'JUMP', 'BIG JUMP'] + list(postSB2)

years = ['', '', '', '', ''] + years

trace61 = go.Scatter(name = 'Movement After Super Bowl', x = years, y = postSB2,
                     mode = 'lines+markers',
                     marker = dict(size = 12, color = 'Green'))

layout = dict(title = 'Movement in the NASDAQ After the Super Bowl',
              ...
              yaxis = dict(title = 'Change Description'))
...
group_labels = ['Sturm & Ruger Distribution Plot']


layout = dict(title = 'Sturm & Ruger Distribution Plot',
              xaxis = dict(title = 'Change Size (Percentage)'),
